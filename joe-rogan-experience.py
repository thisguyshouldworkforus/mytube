...
from libs.functions import ProofOfLife, CheckHistory, CheckProcess, JREFileName, LoggIt, NotifyMe, WriteHistory, PlexLibraryUpdate

####[ REQUIRED VARIABLES ]####
LOGGER = str('jre')
OUTPUT_PATH = str(pytubefix.helpers.target_directory('/opt/media/tv.docs/The Joe Rogan Experience (2009) {tvdb-326959}'))
SERIES_PREFIX = str('The Joe Rogan Experience (2009) - ')
YOUTUBE_URL = str('https://www.youtube.com/c/joerogan/videos')
SECTION_ID = str('6')
...
def main():
    ...
    for index, VID in enumerate(x.videos, start=1):
        ...
        if re.search(pattern, TITLE, re.IGNORECASE):
            LoggIt(LOGGER, f"Episode \"{TITLE}\" ({ID}) is not a desired episode!")
            if not CheckHistory(HISTORY_LOG, VIDEO):
                WriteHistory(HISTORY_LOG, VIDEO)
                continue
            else:
        ...
        if LENGTH < 59:
            LoggIt(LOGGER, f"Episode \"{TITLE}\" ({ID}) is too short ({LENGTH} minutes!)")
            if (not(CheckHistory(HISTORY_LOG, VIDEO))):
                WriteHistory(HISTORY_LOG, VIDEO)
                continue
            else:
