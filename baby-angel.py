...
####[ REQUIRED VARIABLES ]####
LOGGER = str('baby-angel')
OUTPUT_PATH = str(pytubefix.helpers.target_directory('/opt/media/tv.kids/Little Angel (2016) {tvdb-xxxxx}'))
SERIES_PREFIX = str('Little Angel (2016) - ')
YOUTUBE_URL = str('https://www.youtube.com/@LittleAngel/videos')
SECTION_ID = str('7')
SERIES_URL = str('http://plex.int.snyderfamily.co:32400/web/index.html#!/server/50d6b668401e93d23054d59158dfff33bc988de4/details?key=%2Flibrary%2Fmetadata%2F53772&context=source%3Acontent.library~2~1')
PLAYLIST = False
CHANNEL = True
INITIALIZE = True
####[ REQUIRED VARIABLES ]####
...
def main():
    ...
    for index, VID in enumerate(x.videos, start=1):
        ...
        # Only interested in short-form videos
        if LENGTH > 6:
            LogIt(LOGGER, f"Episode \"{TITLE}\" ({ID}) is too long ({LENGTH} minutes!)", "info")
            if (not(CheckHistory(HISTORY_LOG, VIDEO))):
                WriteHistory(HISTORY_LOG, VIDEO)
                continue
