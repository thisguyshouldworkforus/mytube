...
def main():
    ...
    for index, VIDEO in enumerate(VIDEOS, start=1):
        ...
        if (not(CheckHistory(HISTORY_LOG, VIDEO))):
            ...
            LoggIt(LOGGER, f"{index} of {len(VIDEOS)}: \"{TITLE}\" ({ID}) was NOT in history, and will be downloaded.")
            ...
            LoggIt(LOGGER, f"{index} of {len(VIDEOS)}: \"{TITLE}\" ({ID}) WAS in history, and will be disgarded.")
            continue
