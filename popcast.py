...
####[ REQUIRED VARIABLES ]####
LOGGER = str('popcast')
OUTPUT_PATH = str(pytubefix.helpers.target_directory('/opt/media/tv.docs/Popcast (2018) {tvdb-000000}'))
SERIES_PREFIX = str('Popcast (2018) - ')
YOUTUBE_URL = str('https://www.youtube.com/@ThePopcast/videos')
SECTION_ID = str('6')
SERIES_URL = str('http://plex.int.snyderfamily.co:32400/web/index.html#!/server/50d6b668401e93d23054d59158dfff33bc988de4/details?key=%2Flibrary%2Fmetadata%2F51906&context=source%3Acontent.library~0~0')
PLAYLIST = False
CHANNEL = True
INITIALIZE = False
####[ REQUIRED VARIABLES ]####
...
def main():
    ...
    for index, VID in enumerate(x.videos, start=1):
        ...
        if LENGTH < 29:
            LogIt(LOGGER, f"Episode \"{TITLE}\" ({ID}) is too short ({LENGTH} minutes!)")
            if (not(CheckHistory(HISTORY_LOG, VIDEO))):
                WriteHistory(HISTORY_LOG, VIDEO)
                continue
