...
def main():
    ...
    workbook = xlsxwriter.Workbook(f'{os.path.dirname(__file__)}/youtube_stats.{TODAY}.xlsx')
    ...
    # Write the video array to a file, as a backup.
    ...
    if TOTAL >= 60:
        ...
        MINUTES = TOTAL % 60
        if HOURS == 1:
            logger.info(f"\n>> Total Time: {HOURS} hour, {MINUTES} minutes <<\n")
            print(f"\n>> Total Time: {HOURS} hour, {MINUTES} minutes <<\n")
        else:
